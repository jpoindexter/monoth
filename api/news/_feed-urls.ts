...
export function getFeedUrls(category: string, region = 'global'): { name: string; url: string }[] {
  ...
  const feeds: Record<string, { name: string; url: string }[]> = {
    ...
    geopolitics: [
      ...
      { name: 'Russia', url: gn('Russia economy OR "Russia sanctions" OR ruble OR Gazprom OR "Russia war" when:2d', r) },
      { name: 'China', url: gn('China economy OR "Xi Jinping" OR PBOC OR yuan OR "China trade" when:2d', r) },
      { name: 'India', url: gn('India economy OR "India GDP" OR Modi OR "Reserve Bank of India" OR rupee when:3d', r) },
      { name: 'Middle East', url: gn('"Middle East" conflict OR Iran OR "oil supply" OR OPEC when:2d', r) },
      { name: 'BRICS', url: gn('BRICS OR "emerging markets" OR "de-dollarization" when:3d', r) },
      { name: 'Korea/Japan', url: gn('"North Korea" OR "South Korea" OR "Bank of Japan" OR "Japan economy" when:3d', r) },
      { name: 'Europe', url: gn('Europe economy OR ECB OR "European Union" OR euro OR NATO when:2d', r) },
      { name: 'Latin America', url: gn('"Latin America" OR Brazil OR Mexico OR Argentina OR "South America" economy when:3d', r) },
      { name: 'Africa', url: gn('Africa economy OR Nigeria OR "South Africa" OR Kenya OR "sub-saharan" when:3d', r) },
      { name: 'Australia/NZ', url: gn('Australia economy OR "Reserve Bank of Australia" OR "New Zealand" economy when:3d', r) },
      { name: 'Canada', url: gn('Canada economy OR "Bank of Canada" OR CAD OR "Canadian dollar" when:3d', r) },
      { name: 'Southeast Asia', url: gn('"Southeast Asia" OR ASEAN OR Vietnam OR Indonesia OR Thailand economy when:3d', r) },
    ],
    realestate: [
